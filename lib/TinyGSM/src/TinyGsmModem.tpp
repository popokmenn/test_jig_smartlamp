...
class TinyGsmModem {
 ...
 public:
  ...
  bool isNetworkConnected2g() {
    return thisModem().isNetworkConnected2gImpl();
  }
  ...
  bool waitForNetwork2g(uint32_t timeout_ms = 60000L, bool check_signal = false) {
    return thisModem().waitForNetworkImpl2g(timeout_ms, check_signal);
  }
  ...
  int8_t waitResponseImpl(uint32_t timeout_ms, String& data,
    ...
    do {
      ...
      while (thisModem().stream.available() > 0) {
        ...
        else if (thisModem().handleQMTs(data)) {
          data = "";
        }
      }
    } while (millis() - startMillis < timeout_ms);
 ...
 protected:
  ...
  bool waitForNetworkImpl2g(uint32_t timeout_ms   = 60000L,
                          bool     check_signal = false) {
    for (uint32_t start = millis(); millis() - start < timeout_ms;) {
      if (check_signal) { thisModem().getSignalQuality(); }
      if (thisModem().isNetworkConnected2g()) { return true; }
      delay(250);
    }
    return false;
  }
