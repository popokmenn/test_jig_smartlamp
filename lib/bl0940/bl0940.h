...
#define BL0940_WRITE_CMD                    0xA8    //0xA8 in docs. Many thanks to Theo Arends @ https://github.com/arendst/ 
...
#define BL0940_VOLTAGE_DIVIDER              3.8245//1.995           // Blitzwolf BW-SHP10 socket
...
{
    public:
        bl0940( HardwareSerial* hwSerial, HardwareSerial* hwSerial2 );
        // bl0940();
        ~bl0940();
        ...
        uint32_t getCF_CNT();
        // float getEnergy();
        float getEnergy(uint32_t cf, uint16_t calibrated_blink_second);
        // float getEnergyDelta();
        float getPhaseAngle();
        float getPhaseAngleMod();
        float getPowerFactor(bool percentage=true);
        float getPowerFactorMod(bool percentage=true);
