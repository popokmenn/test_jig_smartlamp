 ...
 *
 */
bl0940::bl0940(HardwareSerial *hwSerial, HardwareSerial *hwSerial2)
{
    this->_serial = hwSerial;
    this->_serial2 = hwSerial2;
    static_cast<HardwareSerial *>(_serial)->begin(BL0940_DEFAULT_BAUD_RATE, BL0940_DEFAULT_PORT_CONFIG, 26, 25);
    _init();
}
...
// {
//     this->_serial = hwSerial;
//     static_cast<HardwareSerial*>(_serial)->begin(BL0940_DEFAULT_BAUD_RATE, BL0940_DEFAULT_PORT_CONFIG,25,26);
//     _init();
// }
...
float bl0940::getVoltage()
{
    return (((uint32_t)_rawHolder[BL0940_VRMS_HOLD_POS + 2] << 16) |
           ...
           (float)BL0940_V_RMS_COEFFICIENT * (float)BL0940_VOLTAGE_DIVIDER;
}
...
float bl0940::getCurrent()
{
    return (((uint32_t)_rawHolder[BL0940_IRMS_HOLD_POS + 2] << 16) |
           ...
           (float)BL0940_I_RMS_COEFFICIENT / (float)BL0940_SHUNT_RESISTANCE;
}
...
double bl0940::getActivePower()
{
    ...
    activePow = (double)powerValue * (float)BL0940_WATT_COEFFICIENT * (float)BL0940_VOLTAGE_DIVIDER / (float)BL0940_SHUNT_RESISTANCE;
    return activePow;
}
 ...
 */
double bl0940::getReactivePower()
{
    return reactivePow;
}
 ...
 * Calculations from Power Triangle
 */
double bl0940::getApparentPower()
{
    return appPow;
}

uint32_t bl0940::getCF_CNT()
{
    return (((uint32_t)_rawHolder[BL0940_CF_CNT_HOLD_POS + 2] << 16) | ((uint32_t)_rawHolder[BL0940_CF_CNT_HOLD_POS + 1] << 8) | ((uint32_t)_rawHolder[BL0940_CF_CNT_HOLD_POS]));
}
 ...
 */
float bl0940::getEnergy(uint32_t cf, uint16_t calibrated_blink_second)
{
    if (cf == 0)
    {
        Serial.println("CF count is zero, check sensor connection.");
    }
    return (float)cf * ((BL0940_ACTIVE_ENERGY_COEFFICIENT * BL0940_WATT_COEFFICIENT * (float)BL0940_VOLTAGE_DIVIDER) * 6120 / (6120 * 3600000.0 * (float)BL0940_SHUNT_RESISTANCE));
}
...
{
    // pAngle = 2*PI*((uint16_t)_rawHolderPA[1]<<8 | _rawHolderPA[0])*(float)_cfg.acFreq/(float)BL0940_SAMPLING_FREQUENCY;
    // return pAngle;
...
float bl0940::getPowerFactor(bool percentage)
{
    ...
    // return cos(pAngle) * (double)((percentage) ? 100 : 1);
    // return pwf;
}

float bl0940::getPowerFactorMod(bool percentage)
{
    // return cos(getPhaseAngle()) * (float)((percentage) ? 100 : 1);
    return cos(pAngle) * (double)((percentage) ? 100 : 1);
    // return pwf;
...
float bl0940::getTemperature()
{
    return BL0940_TEMPERATURE_COEFFICIENT *
           ((float)(((uint32_t)(_rawHolder[BL0940_TPS1_HOLD_POS + 1] & 0x0F) << 8) | ((uint32_t)_rawHolder[BL0940_TPS1_HOLD_POS])) / 2 - 45);
}
...
{
    if ((val != BL0940_RMS_REG_UPDATE_RATE_400MS) && (val != BL0940_RMS_REG_UPDATE_RATE_800MS))
        return;
...
{
    if ((val != BL0940_AC_FREQ_50HZ) && (val != BL0940_AC_FREQ_60HZ))
        return;
...
{
    if ((val != BL0940_CF_ENERGY_PULSE) && (val != BL0940_CF_ALARM_PULSE))
        return;
...
{
    if ((val != BL0940_TEMPERATUTE_CTRL_ON) && (val != BL0940_TEMPERATUTE_CTRL_OFF))
        return;
...
{
    if ((val != BL0940_TEMPERATURE_ALARM_ON) && (val != BL0940_OVERCURRENT_ALARM_ON))
        return;
...
{
    if ((val != BL0940_TEMPERATURE_AUTO) && (val != BL0940_TEMPERATURE_SAME) && (val != BL0940_TEMPERATURE_INTERNAL) && (val != BL0940_TEMPERATURE_EXTERNAL))
        return;
...
{
    if ((val != BL0940_TEMPERATURE_INTERVAL_50MS) && (val != BL0940_TEMPERATURE_INTERVAL_100MS) && (val != BL0940_TEMPERATURE_INTERVAL_200MS) && (val != BL0940_TEMPERATURE_INTERVAL_400MS))
        return;
...
{
    if ((BL0940_I_FAST_RMS_CTRL_REG_ADDR > regAddress) || (BL0940_TPS2_B_REG_ADDR < regAddress))
        return false;
    _unlockWRProt();
    if (_cfg.wprot)
        return false;
...
{
    if (((millis() - _frame.lastRcv) < _cfg.rmsUpdate) && (_frame.Payload[BL0940_FRM_ADDR_POS] == BL0940_READALL_REG_ADDR))
        return true;
...
bool bl0940::readValuesPhAngle()
{
    long regValue = 0;

    // _frame.Payload[BL0940_FRM_HEAD_POS] = BL0940_READ_CMD;
    // _frame.Payload[BL0940_FRM_ADDR_POS] = BL0940_CORNER_REG_ADDR;
    uint8_t payloadPhAngle[2] = {0x58, 0x0c};
    _serial->write(payloadPhAngle, 2);
    // _sendFrame();
    _rcvFrameSingle();
    // if( _state == NO_ERROR )
    // {
    ...
    // }
    // if(regValue < BL0940_MAX_REG_VALUE)
    //     {
    //       _rawHolder[BL0940_CORNER_HOLD_POS] = ( regValue & 0xFF );
    //       _rawHolder[BL0940_CORNER_HOLD_POS + 1] = ( regValue>>8 & 0xFF );
    //     }
    return 1;
}

void bl0940::calcEnergy()
{

    pwf = cos(pAngle) * (double)100.0;
    reactivePow = activePow * tan(pAngle);
    appPow = (activePow * 100) / pwf;
}
...
void bl0940::_rcvFrameSingle()
{
    int bytesAvailable;
    uint8_t payloadReceived[40];
    _frame.lastRcv = millis();

    while (!((millis() - _frame.lastRcv) > BL0940_TIMEOUT))
    {
        bytesAvailable = _serial->available();

        if (bytesAvailable > 0)
        {
            // _serial2->print("ba:");_serial2->println(bytesAvailable);
            for (int bytesIndex = 0; bytesIndex < bytesAvailable; bytesIndex++)
            {
                _rawHolderPA[bytesIndex] = (uint8_t)_serial->read();
                //  _serial2->print("pl:");_serial2->print(bytesIndex);_serial2->println(payloadReceived[bytesIndex]);
                // _frame.Payload[(_frame.Index)+bytesIndex] = (uint8_t)_serial->read();
            }
            _frame.lastRcv = millis(); // Reset timeout counter
        }
    }
    // memcpy( (_rawHolder + BL0940_CORNER_HOLD_POS), payloadReceived, 2 );
    // _rawHolderPA[0]= payloadReceived[0];
    // _rawHolder[1]= payloadReceived[1];
}
/*!
 * bl0940::_rcvFrame
...
void bl0940::_rcvFrame()
{
    ...
    {
        _state = BADFRAME_ERR;
        Serial.println("Frame error: Incorrect number of bytes received.");
        Serial.print("Expected: ");
        Serial.println(_frame.Bytes);
        Serial.print("Received: ");
        Serial.println(_frame.Index);
        return;
    }
    ...
    if (BL0940_READALL_REG_ADDR == _frame.Payload[BL0940_FRM_ADDR_POS])
    {
        ...
        {
            _state = CRCFRAME_ERR;
            Serial.println("CRC error: Checksum mismatch.");
            Serial.print("Expected CRC: ");
            Serial.println(_frame.Payload[BL0940_FRM_SEND_MODE_HEAD_POS + BL0940_SEND_MODE_FRAME_BYTES - 1], HEX);
            Serial.print("Calculated CRC: ");
            Serial.println(_crcCalc(_frame.Payload, _frame.Bytes), HEX); // Assuming _crcCalc returns the calculated CRC
            return;
        }
        memcpy(_rawHolder, (_frame.Payload + BL0940_FRM_SEND_MODE_HEAD_POS + BL0940_IFRMS_FRM_POS), 6);
        memcpy((_rawHolder + BL0940_VRMS_HOLD_POS), (_frame.Payload + BL0940_FRM_SEND_MODE_HEAD_POS + BL0940_VRMS_FRM_POS), 3);
        memcpy((_rawHolder + BL0940_WATT_HOLD_POS), (_frame.Payload + BL0940_FRM_SEND_MODE_HEAD_POS + BL0940_WATT_FRM_POS), 3);
        memcpy((_rawHolder + BL0940_CF_CNT_HOLD_POS), (_frame.Payload + BL0940_FRM_SEND_MODE_HEAD_POS + BL0940_CF_CNT_FRM_POS), 3);
        memcpy((_rawHolder + BL0940_TPS1_HOLD_POS), (_frame.Payload + BL0940_FRM_SEND_MODE_HEAD_POS + BL0940_TPS1_FRM_POS), 2);
        memcpy((_rawHolder + BL0940_TPS2_HOLD_POS), (_frame.Payload + BL0940_FRM_SEND_MODE_HEAD_POS + BL0940_TPS2_FRM_POS), 2);
    }
    else
    {
        if (!_crcCalc(_frame.Payload, _frame.Bytes))
        {
            ...
            Serial.println("CRC error: Checksum mismatch.");
            Serial.print("Expected CRC: ");
            Serial.println(_frame.Payload[_frame.Bytes - 1], HEX);
            Serial.print("Calculated CRC: ");
            Serial.println(_crcCalc(_frame.Payload, _frame.Bytes), HEX); // Assuming _crcCalc returns the calculated CRC
            return;
        }
